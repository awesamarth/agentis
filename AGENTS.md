...
- Hosted `wallet list` groups enabled wallets by agent with current API name (login name when unavailable). JSON is `{name, agentId, wallets: [{walletId, chainId, address}]}[]`, without policy/setup internals. Human hosted/local lists share bold cyan names, bold Chains and left-aligned addresses; agent blocks have two blank lines after them. Note lines have a blank line before/after. History shows the most recent entries oldest → newest.
