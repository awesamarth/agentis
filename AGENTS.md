...
- Profile is now `/profile`; `/dashboard/profile` was removed, with no redirect. V1-style identity, overview and spend analytics use owner-scoped backend aggregates (all settled records, UTC daily buckets, fees included). API access lives on `/dashboard/agents/[id]`, not profile: 24-hour keys default to all of that agent’s enabled networks (including future additions), with an optional specific-network restriction and copy/revoke controls. Existing wallet-scoped keys retain their scope. Scoped keys can list only their enabled wallets; execution rechecks scope. Account-wide keys remain unavailable. Typechecks, targeted lint and the isolated aggregate/authorization regression pass; authenticated profile interaction is unverified.
...
- Ports: dashboard 3000, API 3001, **Postgres 127.0.0.1:55432** (Compose project `agentis-rewrite`, database `agentis_dev`), x402 fixture 4000, MPP fixture 4001. Migrations through `0006_agent_access_keys.sql` applied locally. Check live processes/health; don't trust historical PIDs.
