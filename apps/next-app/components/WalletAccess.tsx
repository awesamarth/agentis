...
export default function WalletAccess({ agentId }: { agentId: string }) {
  ...
  const action = useMutation({ mutationFn: async (input: { revokeId: string } | { expiresAt: string }) => {
    ...
    const key = await client.grants.create({ agentId, ...(scope === 'all' ? {} : { chainIds: selectedChains }), agentName: agent.name, expiresAt: input.expiresAt })
  ...
  return <section id="api-access" className="scroll-mt-8">
    ...
    <div className="space-y-5 border border-beige-darker bg-white p-6">
      <p className="font-mono text-xs leading-relaxed text-ink-muted">Connect this agent through the SDK or CLI. Keys expire after 24 hours and follow this agent’s rules. They cannot access other agents, change settings or approve payments. All networks includes networks you enable later; specific selections do not.</p>
