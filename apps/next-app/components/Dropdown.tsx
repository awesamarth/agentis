...
export default function Dropdown({ label, value, onChange, options, disabled = false, className = '' }: {
  ...
  return <div className={`min-w-0 ${className}`}>
    ...
    <Select.Root value={value} onValueChange={onChange} disabled={disabled || options.length === 0}>
      <Select.Trigger ref={setTrigger} id={id} className="mt-2 flex w-full items-center justify-between gap-3 border border-beige-darker bg-[#faf7f1] p-3 text-left font-mono text-sm text-ink outline-none focus-visible:ring-2 focus-visible:ring-ink disabled:cursor-not-allowed disabled:opacity-40 [&>span:first-child]:truncate">
      ...
      <Select.Portal container={trigger?.closest('dialog') ?? undefined}>
        <Select.Content data-agentis-dropdown="" position="popper" sideOffset={4} collisionPadding={16} className="z-50 max-h-[var(--radix-select-content-available-height)] w-[var(--radix-select-trigger-width)] max-w-[calc(100vw-2rem)] overflow-hidden border border-beige-darker bg-[#faf7f1] font-mono text-sm text-ink shadow-lg">
          <Select.Viewport>
