...
export default function Onboarding({ agentId }: { agentId?: string } = {}) {
  const { authenticated, user, getAccessToken } = usePrivy()
  return authenticated && user ? <Setup key={user.id} ownerId={user.id} getAccessToken={getAccessToken} agentId={agentId} /> : null
}
...
function Setup({ ownerId, getAccessToken, agentId }: { ownerId: string; getAccessToken: () => Promise<string | null>; agentId?: string }) {
  ...
  useEffect(() => { if (!agentId && agents.data?.length === 0 && !opened.current) { opened.current = true; requestId.current = crypto.randomUUID(); dialog.current?.showModal() } }, [agents.data, agentId])
  ...
  return <section className="space-y-5">
    ...
    <dialog ref={dialog} aria-labelledby="setup-title" onCancel={event => { if (save.isPending) event.preventDefault() }} onClick={event => {
      ...
      {!finished && <ol aria-label="Setup progress" className="my-7 grid shrink-0 grid-cols-3 gap-3">{['Agent', 'Rules', 'Review'].map((label, index) => <li key={label} aria-current={step === index ? 'step' : undefined} className={`font-mono text-xs uppercase ${index <= step ? 'border-black' : 'border-beige-darker text-ink-muted'}`}>{canSave ? <button type="button" disabled={save.isPending} onClick={() => { setFormError(''); setStep(index) }} className="block w-full border-t-2 border-inherit pt-3 text-left uppercase hover:underline disabled:opacity-40">0{index + 1} · {label}</button> : <span className="block border-t-2 border-inherit pt-3">0{index + 1} · {label}</span>}</li>)}</ol>}
