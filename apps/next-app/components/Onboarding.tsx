...
const labels = { perTransaction: 'Per payment', hourly: 'Per hour', daily: 'Per day', total: 'Total budget' }
...
function Setup({ ownerId, getAccessToken }: { ownerId: string; getAccessToken: () => Promise<string | null> }) {
  ...
  return <section className="space-y-5">
    ...
    <dialog ref={dialog} aria-labelledby="setup-title" onCancel={event => { if (save.isPending) event.preventDefault() }} className="fixed inset-0 m-auto max-h-[90dvh] w-[calc(100%_-_2rem)] max-w-2xl overflow-y-auto border border-beige-darker bg-beige p-0 text-ink shadow-xl backdrop:bg-black/50">
      <div className="p-6 sm:p-9"><header className="flex items-start justify-between gap-4"><div><p className="font-mono text-xs uppercase tracking-widest text-ink-muted">{editing ? 'Agent settings' : 'New agent'}</p><h2 id="setup-title" className="mt-3 font-serif text-3xl font-bold">{finished ? `${name} is ready.` : editing ? `Configure ${name}` : 'Create your agent.'}</h2></div><button aria-label="Close setup" className="p-2 text-2xl" disabled={save.isPending} onClick={() => dialog.current?.close()}>×</button></header>
      {!finished && <ol aria-label="Setup progress" className="my-7 grid grid-cols-3 gap-3">{['Agent', 'Rules', 'Review'].map((label, index) => <li key={label} aria-current={step === index ? 'step' : undefined} className={`border-t-2 pt-3 font-mono text-xs uppercase ${index <= step ? 'border-black' : 'border-beige-darker text-ink-muted'}`}>0{index + 1} · {label}</li>)}</ol>}
      {!finished && step === 0 && <div className="space-y-4"><label className="block text-sm">Agent name<input className={field} value={name} maxLength={80} placeholder="Research agent" onChange={event => setName(event.target.value)} /></label><p className="text-sm text-ink-muted">Choose this agent’s networks. Other agents keep their own wallets.</p>{query.data?.networks.map(network => <label key={network.key} className="flex cursor-pointer items-center gap-3 border border-beige-darker p-3"><input type="checkbox" className="accent-black" checked={selected.includes(network.key)} onChange={event => { const next = event.target.checked ? [...selected, network.key] : selected.filter(key => key !== network.key); setSelected(next); if (!next.includes(defaultNetwork)) setDefaultNetwork(next[0] ?? 'base') }} /><span className="flex-1 font-serif text-lg font-bold">{network.name}</span><span className="text-xs text-ink-muted">{network.testnet ? 'Test network' : 'Mainnet'}</span></label>)}<label className="block text-sm">Default network<span className="relative mt-2 block"><select className={selectField} value={defaultNetwork} onChange={event => setDefaultNetwork(event.target.value)}>{query.data?.networks.filter(network => selected.includes(network.key)).map(network => <option key={network.key} value={network.key}>{network.name}</option>)}</select><ChevronDown size={16} aria-hidden="true" className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2" /></span></label></div>}
