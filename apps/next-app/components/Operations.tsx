  ...
  return <section className="space-y-4">
    ...
    {query.data?.map(operation => {
      ...
      return <article className="border border-beige-darker bg-[#faf7f1] px-4 py-3" key={operation.id}>
        ...
        {operation.reason && <p className="mt-1.5 break-words text-xs">{operation.reason}</p>}
        {operation.payment && <p className="mt-1.5 break-all font-mono text-xs">x402 · GET {operation.payment.url}</p>}
        {operation.mpp && <p className="mt-1.5 break-all font-mono text-xs">MPP · GET {operation.mpp.url}</p>}
        {operation.httpResponse && <p className="mt-1.5 text-xs text-ink-muted">API response: HTTP {operation.httpResponse.status}. Payment settlement is tracked separately.</p>}
        <dl className="mt-2 flex flex-wrap gap-x-5 gap-y-1.5 text-xs"><div className="flex w-full min-w-0 items-baseline gap-2"><dt className="shrink-0 text-ink-muted">Recipient</dt><dd className="min-w-0 break-all font-mono">{operation.to}</dd></div><div className="flex flex-wrap gap-x-2"><dt className="text-ink-muted">Maximum network fee</dt><dd className="break-all">{network ? `${formatUnits(BigInt(operation.maxFeeAtomic), network.decimals)} ${network.currency}` : `${operation.maxFeeAtomic} atomic units`}</dd></div>{operation.usdReservedMicros != null && <div className="flex flex-wrap gap-x-2"><dt className="text-ink-muted">{operation.usdSettledMicros != null ? 'Budget charged' : 'Maximum USD cost · including fees'}</dt><dd className="break-all">${formatUnits(BigInt(operation.usdSettledMicros ?? operation.usdReservedMicros), 6)}</dd></div>}</dl>
        {operation.status === 'pending_approval' && <p className="mt-2 text-xs text-ink-muted">Approval expires {new Date(operation.expiresAt).toLocaleString()}.</p>}
        {(approving || processing) && <p role="status" className="mt-2 flex items-center gap-2 text-xs text-ink"><LoaderCircle size={14} className="motion-safe:animate-spin" aria-hidden="true" />{approving ? 'Approving payment…' : 'Processing transaction…'}</p>}
        {operation.status === 'unknown' && <p role="status" className="mt-2 border-l-2 border-accent pl-2 text-xs">We’re checking whether this payment went through. Don’t send it again.</p>}
        {operation.error && <p className="mt-2 break-words text-xs">{operation.error}</p>}
