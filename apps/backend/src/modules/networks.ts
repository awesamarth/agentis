...
export const networkKey = z.enum(['base', 'arc', 'tempo', 'solana'])
export const networkSelection = z.object({ networks: z.array(networkKey).min(1).max(4).refine(items => new Set(items).size === items.length, 'Duplicate network'), defaultNetwork: networkKey }).strict().refine(value => value.networks.includes(value.defaultNetwork), 'Default network must be enabled')
export const defaultProductChain = `eip155:${baseSepolia.id}`
export const baseUsdc = '0x036cbd53842c5426634e7929541ec2318f3dcf7e'
export const evmChains = [baseSepolia, arcTestnet, tempoTestnet] as const
...
export function evmClient(chainId: string) {
  ...
  const envKey = chain.id === baseSepolia.id ? 'BASE_SEPOLIA_RPC_URL' : chain.id === arcTestnet.id ? 'ARC_TESTNET_RPC_URL' : 'TEMPO_TESTNET_RPC_URL'
