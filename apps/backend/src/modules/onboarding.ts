...
import { agentSettings, pluginIds, walletPolicy, type UsdLimits } from '@agentis-hq/core/operations'
...
export function onboardingRoutes(service: OperationService, identity: Identity) {
  ...
  const save: Handler<Env> = async c => {
    ...
    return c.json(await service.db.transaction(async tx => {
      ...
      if (!creating && currentAgent && !input.enableExecution && isDeepStrictEqual({ ...currentAgent, name: values.name, plugins: values.plugins }, values)) {
        await tx.update(agents).set({ name: values.name, plugins: values.plugins }).where(and(eq(agents.id, input.id), eq(agents.ownerId, ownerId)))
        return view(values)
      }
  ...
  app.patch('/agents/:id/plugins', async c => {
    ...
    const { plugins } = z.object({ plugins: pluginIds }).strict().parse(await c.req.json())
    const ownerId = c.get('principal').ownerId
    return c.json(await service.db.transaction(async tx => {
      await tx.execute(sql`select pg_advisory_xact_lock(hashtextextended(${`owner:${ownerId}`}, 0))`)
