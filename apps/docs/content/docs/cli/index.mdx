...
The CLI uses OAuth authorization code flow with PKCE and stores its access and
refresh credentials in the OS keychain. Each CLI login is independently
revocable and does not invalidate other sessions.
