...
export const localNetworks = {
  sepolia: { name: 'Ethereum Sepolia', chainId: `eip155:${sepolia.id}`, chain: sepolia, rpcEnv: 'SEPOLIA_RPC_URL', assets: { ETH: { decimals: 18, token: null }, USDC: { decimals: 6, token: '0x1c7d4b196cb0c7b01d743fbc6116a902379c7238' } } },
...
export function parseChains(input: string): LocalChain[] {
  ...
  if (!chains.length || chains.some(chain => !Object.hasOwn(localNetworks, chain)) || new Set(chains).size !== chains.length) throw Error('Choose unique chains: base, arc, tempo, solana, sepolia')
