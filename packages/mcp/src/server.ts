...
function requireAccessToken(): string {
  if (!accessToken?.startsWith('agt_user_') && !accessToken?.startsWith('agt_oauth_')) {
    throw new Error('Agentis MCP requires an account key or OAuth access token')
  }
  return accessToken
}
...
async function getMainnetUsdcBalancesAtomic(walletAddresses: string[]): Promise<Map<string, bigint>> {
  const mainnetRpc = options.mainnetRpcUrl ?? DEFAULT_MAINNET_RPC
...
server.registerTool(
  ...
  async () => result({
    command: 'agentis',
    help: 'Run `agentis --help` for CLI command discovery. MCP tools are available directly through this server.',
  }),
)
...
server.registerTool(
  'agentis_privacy_create_utxo',
  {
    ...
    description: 'Create a receiver-claimable Umbra UTXO from encrypted balance. Amount is atomic units.',
    inputSchema: {
      agent: agentRef,
