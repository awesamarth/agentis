...
export type Delegation = { agentId: string; name: string; client: AgentisClient }
...
export function createAgentisMcpServer(options: { delegations: Delegation[]; networks: readonly Network[] }) {
  ...
  server.registerTool('agentis_list_wallets', { description: 'List connected agents and their authorized enabled wallets. Use walletId for payments and agentId for balance/policy/history.', inputSchema: {}, annotations: read }, () => run(async () => Promise.all(delegations.map(async item => ({ name: item.name, agentId: item.agentId, wallets: (await item.client.wallets.list()).filter(wallet => wallet.enabled).map(wallet => ({ walletId: wallet.id, chainId: wallet.chainId, address: wallet.address, assets: networks.find(network => network.chainId === wallet.chainId)?.assets })) })))))
  ...
  server.registerTool('agentis_fetch', {
    ...
    inputSchema: { walletId: z.string().uuid(), url: z.string().url().max(4096), maxAmount: decimal.describe('Maximum price in decimal USDC/alphaUSD units'), maxFee: decimal.optional().describe('Tempo only: decimal protocol USD fee budget, defaults to 0.01'), idempotencyKey: key }, annotations: payment,
  }, ({ walletId, url, maxAmount, maxFee, idempotencyKey }) => run(async () => {
    const item = await forWallet(walletId)
    return item.client.fetch({ walletId, url, maxAmountAtomic: atomic(maxAmount, 6), ...(item.wallet.chainId === 'eip155:42431' ? { maxFeeAtomic: atomic(maxFee ?? '0.01', 18) } : {}) }, { idempotencyKey })
  }))
